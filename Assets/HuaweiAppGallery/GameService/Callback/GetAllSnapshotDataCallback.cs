...
{
    public class GetAllSnapshotDataCallback : AndroidJavaProxy
    {
        private IGetAllSnapshotDataListener _listener;

        public GetAllSnapshotDataCallback(IGetAllSnapshotDataListener listener) : base(
            "com.unity.udp.extension.sdk.games.extra.ExtrasCallback$OnGetAllSnapshotData")
        {
            _listener = listener;
        }

        public void onSuccess(AndroidJavaObject allSnapshotData)
        {
            ...
            if (_listener != null)
            {
                int size = allSnapshotData.Call<int>("size");
                List<SnapshotData> list = new List<SnapshotData>();
                for (int i = 0; i < size; i++)
                {
                    AndroidJavaObject jo = allSnapshotData.Call<AndroidJavaObject>("get", i);
                    ...
                    if (gameJo != null)
                    {
                        ...
                        AndroidJavaObject gameHiResImageJo = gameJo.Call<AndroidJavaObject>("getHiResImageUri");
                        if (gameHiResImageJo != null)
                        {
                            game.HiResImageUri = gameHiResImageJo.Call<string>("toString");
                        }

                        AndroidJavaObject gameIconImageJo = gameJo.Call<AndroidJavaObject>("getIconImageUri");
                        if (gameIconImageJo != null)
                        {
                            game.IconImageUri = gameIconImageJo.Call<string>("toString");
                        }
